...
async function startManagedLightpanda(options: NormalizedOptions): Promise<LightpandaController> {
  ...
  // ⚡ Bolt: Use AbortController to actively cancel the pending port polling
  // background task when the race resolves or rejects early.
...
function waitForPort(options: NormalizedOptions, signal: AbortSignal): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const attempt = () => {
      ...
      try {
        const socket = (activeSocket = net.connect(options.port, options.host));
