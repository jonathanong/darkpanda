...
async function isLightpandaRunning(options: NormalizedOptions): Promise<boolean> {
  ...
  try {
    return await new Promise((resolve) => {
      ...
      req.on("error", () => {
        // ⚡ Bolt: Explicitly destroy socket on error to prevent file descriptor leaks
        // and keep-alive issues when the service is unreachable.
        req.destroy();
        resolve(false);
      });
      req.on("timeout", () => {
    ...
    const attempt = () => {
      ...
      try {
        ...
        activeSocket.once("error", () => {
          ...
          activeTimer = setTimeout(attempt, 25);
          activeTimer.unref();
        });
      } catch (err) {
        finish(err as Error);
