...
async function startManagedLightpanda(options: NormalizedOptions): Promise<LightpandaController> {
  ...
  // ⚡ Bolt: Use AbortController with Promise.race to cancel port polling
  // if the process errors early, preventing background resource leaks.
...
function waitForPort(options: NormalizedOptions, signal?: AbortSignal): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const finish = (error?: Error) => {
      ...
      if (activeSocket !== undefined && !activeSocket.destroyed) activeSocket.destroy();
      /* v8 ignore next */
