...
describe("Security: HTTP Request Splitting in Lightpanda configuration", () => {
  it("rejects versionPath containing CRLF characters", () => {
    expect(() => {
      normalizeOptions({ versionPath: "/json/version\r\nHost: example.com" });
    }).toThrow("versionPath cannot contain CRLF characters (HTTP Request Splitting risk)");
    ...
    expect(() => {
      normalizeOptions({ versionPath: "/json/version\r" });
    }).toThrow("versionPath cannot contain CRLF characters (HTTP Request Splitting risk)");
  });
});
