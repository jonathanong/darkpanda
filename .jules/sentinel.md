...
## 2024-05-28 - CRLF Injection / HTTP Request Splitting in versionPath

**Vulnerability:** The `options.versionPath` was directly passed to `http.get` without validating against CRLF (`\r`, `\n`) characters. If a user provided a maliciously crafted `versionPath` containing CRLF sequences, it could lead to HTTP Request Splitting, allowing an attacker to inject arbitrary headers or hijack the connection.

**Learning:** When constructing HTTP requests with user-configurable paths (e.g., passing options to `http.get`), Node.js does not always strictly sanitize these inputs natively against CRLF characters, which can expose the application to Request Splitting.

**Prevention:** Always explicitly validate against and reject CRLF characters (`\r`, `\n`) in user-supplied input that will be used in HTTP request paths or headers.
